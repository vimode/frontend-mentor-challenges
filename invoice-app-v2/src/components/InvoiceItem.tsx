...
export default function InvoiceItem({ invoice }) {
	return (
		...
		>
			...
			<p
				className={`flex-1 p-1 rounded-lg text-center font-bold ${invoice.status === "pending" ? "text-status-pending" : invoice.status === "draft" ? "text-status-neutral bg-background-neutral_alt" : "text-status-success"}`}
			>
				<span>&bull;</span>
				{invoice.status}
			</p>
