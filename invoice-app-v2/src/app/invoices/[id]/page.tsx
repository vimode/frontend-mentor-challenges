...
export default async function InvoiceId({
  ...
  const invoice: InvoiceDetails | null = invoicedata
      ...
      <section className="flex md:justify-between md:flex-row flex-col bg-background-secondary p-5 rounded-xl md:place-items-center gap-8">
        <div>
          <p>
            Status
            <span
              className={`ml-4 text-primary_text font-semibold rounded-lg px-4 py-2 ${
                invoice.status === "pending"
                  ? "text-status-pending bg-background-pending_bg"
                  : invoice.status === "paid"
                  ? "text-status-success bg-background-success_bg"
                  : "text-status-neutral bg-background-neutral_bg"
              }`}
            >
              <span>&bull;&nbsp;</span>
      ...
      <main className="bg-background-secondary p-5 rounded-xl">
        <div className="flex flex-col gap-7 md:flex-row md:justify-between">
          <div>
            ...
            <p className="text-secondary_text text-text-secondary">{invoice.description}</p>
          </div>
        ...
        <div className="flex flex-row flex-wrap gap-9 md:gap-28 my-6">
          <div className="flex flex-col gap-3 md:gap-5">
            <div className="flex flex-col gap-1 md:gap-2">
              <p className="text-secondary_text text-text-secondary">Invoice Date</p>
              <p className="text-primary_text font-bold">{formatDateMedFormat(invoice.createdAt)}</p>
            </div>

            <div className="flex flex-col gap-1 md:gap-2">
              <p className="text-secondary_text text-text-secondary">Payment Due</p>
              <p className="text-primary_text font-bold">{formatDateMedFormat(invoice.paymentDue)}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-secondary_text text-text-secondary">
              <span className="text-primary_text text-text font-bold">{invoice.clientName}</span>
              <br />
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-primary_text font-bold">{invoice.clientEmail}</p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="bg-status-neutral flex flex-row justify-between text-background-neutral items-center rounded-b-lg p-5">
            <p>Amount Due</p>
            <p className="text-[28px]">{Number(invoice.total).toFixed(2)}</p>
          </div>
        </div>
      </main>
    </div>
